...
do 
    ...
    Library.Window = function(self, Data)
        ...
        function Window:SetOpen(Bool)
            ...
            local currentPosition = Items["MainFrame"].Instance.Position
            local openTarget = currentPosition
            local openStart = UDim2.new(currentPosition.X.Scale, currentPosition.X.Offset, currentPosition.Y.Scale, currentPosition.Y.Offset + 14)

            if Window.IsOpen then 
                Items["MainFrame"].Instance.Visible = true 
                Items["OpenScale"].Instance.Scale = 0.92
                Items["MainFrame"].Instance.Position = openStart
                TweenService:Create(Items["OpenScale"].Instance, TweenInfo.new(0.42, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1}):Play()
                TweenService:Create(Items["MainFrame"].Instance, TweenInfo.new(0.42, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Position = openTarget}):Play()
            else
                Items["OpenScale"].Instance.Scale = 1
                local closeTarget = UDim2.new(currentPosition.X.Scale, currentPosition.X.Offset, currentPosition.Y.Scale, currentPosition.Y.Offset + 10)
                TweenService:Create(Items["OpenScale"].Instance, TweenInfo.new(0.24, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {Scale = 0.92}):Play()
                TweenService:Create(Items["MainFrame"].Instance, TweenInfo.new(0.24, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {Position = closeTarget}):Play()
            end
        ...
        Library._SliderAnimationConnection = RunService.RenderStepped:Connect(function(delta)
            ...
            for SliderObject, Scanner in pairs(Library._SliderAnimations) do
                if Scanner and Scanner.Parent and SliderObject and SliderObject.SliderFrame then
                    local frame = SliderObject.SliderFrame
                    local min = SliderObject.Min or 0
                    local max = SliderObject.Max or 100
                    local value = SliderObject.Value or min
                    local selected = 0
                    if max ~= min then
...
cheat.utility = {} do
    ...
    cheat.utility.new_hook = function(f, newf, usecclosure) (function()
        if usecclosure then
...
cheat.EspLibrary = {} (function()
    ...
    local esp_no_collision = workspace:FindFirstChild("NoCollision")
    local esp_no_collision_refresh = 0
    local function esp_is_visible(character)
        ...
        if not esp_no_collision or not esp_no_collision.Parent then
            local now = os.clock()
            if now - esp_no_collision_refresh > 1 then
                esp_no_collision = workspace:FindFirstChild("NoCollision")
                esp_no_collision_refresh = now
            end
        end
        if esp_no_collision then filter[#filter + 1] = esp_no_collision end
    ...
    local function create_esp(player, isnpc)
        ...
        local fadethread
        function plr:update_visible_state(force_visibility)
            -- Raycasts are expensive when many ESP entries are on screen. Keep the
            -- visual state responsive, but refresh line-of-sight at a bounded rate.
            local now = os.clock()
            if not settings.visible_check then
                plr._esp_visible = false
            elseif force_visibility or character ~= plr._esp_visibility_character or now - (plr._esp_visibility_tick or 0) >= 0.08 then
                plr._esp_visible = esp_is_visible(character)
                plr._esp_visibility_tick = now
                plr._esp_visibility_character = character
            end
        ...
        plr.connection = cheat.utility.new_renderstepped(function(delta)
            ...
            -- Visibility is throttled internally to avoid repeated raycasts every frame.
            if character and character.Parent then
                plr:update_visible_state(false)
            else
                plr._esp_visible = false
            end
            ...
            if main_settings.maxdistance ~= effective_maxdistance then
...
local function get_tracer_folder()
    local folder = workspace:FindFirstChild("NoCollision")
    if not folder then
...
local function make_beam(Origin, Position, Color, Thickness)
    local tracer_folder = get_tracer_folder()
    local part1 = Instance.new("Part", tracer_folder)
    local part2 = Instance.new("Part", tracer_folder)
    ...
    local Beam = Instance.new("Beam", tracer_folder)
    Beam.Name="Beam"
...
local function create_advanced_tracer(Origin, Position, Color1, Color2, Thickness)
    ...
    local CoreBeam = Instance.new("Beam", tracer_folder)
    ...
    local PulseBeam = Instance.new("Beam", tracer_folder)
...
local function spawn_bullet_tracer(origin, position)
    if not silent_aim.tracer then return end
    if typeof(origin) ~= "Vector3" or typeof(position) ~= "Vector3" then return end
    if (position - origin).Magnitude < 0.05 then return end

    local thickness = math.max(0.05, tonumber(silent_aim.tracer_thickness) or 0.5)
    local lifetime = math.max(0.05, tonumber(silent_aim.tracer_lifetime) or 1)
    local style = silent_aim.tracer_style == "Tracer 2" and "Tracer 2" or "Tracer 1"

    if style == "Tracer 2" then
        local beams, d1, d2 = create_advanced_tracer(origin, position, silent_aim.tracer_color, silent_aim.tracer_color2, thickness)
        local elapsed = 0
        local connection
        connection = cheat.utility.new_renderstepped(function(delta)
            elapsed += delta
            local alpha = math.clamp(elapsed / lifetime, 0, 1)
            local pulse = (math.sin(elapsed * 20) + 1) * 0.5
            for _, beam in ipairs(beams) do
                if beam and beam.Parent then
                    beam.Transparency = NumberSequence.new(alpha)
                    if beam.Name == "PulseBeam" then
                        local width = thickness * (0.5 + pulse)
                        beam.Width0 = width
                        beam.Width1 = width
                    end
                end
            ...
            if elapsed >= lifetime then
                ...
                for _, beam in ipairs(beams) do pcall(function() beam:Destroy() end) end
                pcall(function() d1:Destroy() end)
                pcall(function() d2:Destroy() end)
            end
        end)
    else
        local beam, d1, d2 = make_beam(origin, position, silent_aim.tracer_color, thickness)
        local elapsed = 0
        local connection
        connection = cheat.utility.new_renderstepped(function(delta)
            elapsed += delta
            beam.Transparency = NumberSequence.new(math.clamp(elapsed / lifetime, 0, 1))
            if elapsed >= lifetime then
                if connection then connection:Disconnect() end
...
do
        ...
        if inv_weapon and inv_weapon:FindFirstChild("SettingsModule") then
            ...
            if rawget(weapon_settings,"FireRate") and shoot_debounce<=tick() then
                ...
                if FireProjectile:InvokeServer(as_dir, rnd, autoshootdelay) then
                    ...
                    if silent_aim.tracer and _fire_pos then
                        spawn_bullet_tracer(real_orig, _fire_pos)
                    end
                end
                shoot_debounce = tick() + (rawget(weapon_settings,"FireRate") * speedmult)
            end
    ...
    task.spawn(function() repeat (function()
        ...
        for i = 1, #gc_list do
            ...
            if type(gc)=="table" then
                ...
                if rawget(gc,"CreateBullet") then
                    ...
                    cheat.utility.new_hook(old_bullet, (function(old, self, ...)
                        local args      = {...}
                        ...
                        if silent_aim.enabled then
                            ...
                            -- Tracer: use the same origin/destination that the shot uses.
                            if silent_aim.tracer then
                                local tracer_origin = args[aimpart_index].Position
                                if silent_aim.corner_shoot and silent_aim.manipulated_origin then
                                    tracer_origin = silent_aim.manipulated_origin
                                elseif cheat.freecam_enabled then
                                    local char = LocalPlayer.Character
                                    local head = char and char:FindFirstChild("Head")
                                    if head then tracer_origin = head.Position end
                                end
                                local tracer_target = silent_aim.target_part and silent_aim.target_part.Position
                                if not tracer_target then
                                    tracer_target = tracer_origin + args[aimpart_index].CFrame.LookVector * 10000
                                end
                                spawn_bullet_tracer(tracer_origin, tracer_target)
                            end
                            ...
                            local fire_origin = real_aimpart.Position
                            if silent_aim.corner_shoot and silent_aim.manipulated_origin then
                                fire_origin = silent_aim.manipulated_origin
                            elseif cheat.freecam_enabled then
                                local char = LocalPlayer.Character
                                local head = char and char:FindFirstChild("Head")
                                if head then fire_origin = head.Position end
                            end

                            local target_pos = silent_aim.target_part.Position
                            local delta = target_pos - fire_origin
                            if delta.Magnitude > 0.001 then
                                -- Manipulation must change both the origin and direction.
                                -- Restoring the original CFrame immediately after CreateBullet keeps the viewmodel unchanged.
                                real_aimpart.CFrame = CFrame.new(fire_origin, fire_origin + delta.Unit)
                            end
                            local ok, ret = pcall(old, self, unpack(args, 1, argCount))
                ...
                if rawget(gc,"updateClient") then
                    ...
                    cheat.utility.new_hook(old_update, (function(old,...)
...
local KN_VisualSection = KN_Test10Page:Section({Name = "Visual Presets", Side = 1})
    ...
    local KN_PerfFrames = 0
    local KN_PerfLastUpdate = os.clock()
    local KN_Stats = game:GetService("Stats")
    local KN_PerfNetwork = nil
    local KN_PerfServerStats = nil
    local KN_PerfPingItem = nil
    ...
    KNStateManager:Connect(RunService.RenderStepped, function()
        ...
        KN_PerfFrames += 1
        local now = os.clock()
        local dt = now - KN_PerfLastUpdate
        if dt < 0.5 then return end
        local fps = math.floor(KN_PerfFrames / math.max(dt, 0.001) + 0.5)
        KN_PerfFrames = 0
        KN_PerfLastUpdate = now
        local players = #Players:GetPlayers()
        local ping = "--"
        pcall(function()
            if not KN_PerfNetwork or not KN_PerfNetwork.Parent then
                KN_PerfNetwork = KN_Stats:FindFirstChild("Network")
                KN_PerfServerStats = KN_PerfNetwork and KN_PerfNetwork:FindFirstChild("ServerStatsItem")
                KN_PerfPingItem = KN_PerfServerStats and KN_PerfServerStats:FindFirstChild("Data Ping")
            end
            if KN_PerfPingItem then
                ping = tostring(math.floor(KN_PerfPingItem:GetValue() + 0.5)) .. " ms"
            end
        end)
        pcall(function()
...
-- Lightweight client-side optimizations with reversible toggles.
-- ================================================================
do
    local OptTools = SettingsPage:Section({Name = "Optimizations", Side = 1})
    ...
    local function save_prop(obj, prop)
        ...
        if saved[obj][prop] == nil then
            ...
            if ok then saved[obj][prop] = value end
        end
    end
    ...
    local function optimize_descendants()
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") then
                if Library.Flags.opt_particles then set_prop(obj, "Enabled", false) end
            elseif obj:IsA("PostEffect") then
                if Library.Flags.opt_postfx then set_prop(obj, "Enabled", false) end
            elseif obj:IsA("BasePart") then
                if Library.Flags.opt_shadows then set_prop(obj, "CastShadow", false) end
            end
        end
    end

    OptTools:Label("Client-side FPS optimizations")

    OptTools:Toggle({
        Name = "Low Graphics",
        Flag = "opt_low_graphics",
        Default = false,
        Callback = function(v)
            if v then
                pcall(function()
                    local lighting = game:GetService("Lighting")
                    save_prop(lighting, "GlobalShadows")
                    lighting.GlobalShadows = false
                end)
                for _, obj in ipairs(workspace:GetDescendants()) do
                    if obj:IsA("BasePart") then
                        set_prop(obj, "CastShadow", false)
                    elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") then
                        set_prop(obj, "Enabled", false)
                    elseif obj:IsA("PostEffect") then
                        set_prop(obj, "Enabled", false)
                    end
                end
    ...
    OptTools:Toggle({
        ...
        Callback = function(v)
            ...
            for _, obj in ipairs(lighting:GetChildren()) do
                if obj:IsA("PostEffect") then set_prop(obj, "Enabled", not v) end
            end
        end
    })
    ...
    OptTools:Toggle({
        ...
        Callback = function(v)
            local lighting = game:GetService("Lighting")
            set_prop(lighting, "GlobalShadows", not v)
            for _, obj in ipairs(workspace:GetDescendants()) do
                if obj:IsA("BasePart") then set_prop(obj, "CastShadow", not v) end
            end
        end
    })
    ...
    OptTools:Button({
        ...
        Callback = function()
            optimize_descendants()
            pcall(function()
                local lighting = game:GetService("Lighting")
                if Library.Flags.opt_shadows or Library.Flags.opt_low_graphics then
                    set_prop(lighting, "GlobalShadows", false)
                end
            end)
        end
    })
    ...
    -- Only watches newly-created effects/parts while an optimization is enabled.
    optimizationConnections[1] = Library:Connect(workspace.DescendantAdded, function(obj)
        task.defer(function()
            if not obj or not obj.Parent then return end
            if Library.Flags.opt_particles and (obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam")) then
                set_prop(obj, "Enabled", false)
            elseif Library.Flags.opt_postfx and obj:IsA("PostEffect") then
                set_prop(obj, "Enabled", false)
            elseif (Library.Flags.opt_shadows or Library.Flags.opt_low_graphics) and obj:IsA("BasePart") then
                set_prop(obj, "CastShadow", false)
            end
        end)
    end, "optimization_descendant_added")
end
...
do
    ...
    local function vmchams(force) (function()
...
do
    ...
    local freecam_replication_cache = {}
    task.spawn(function()
            ...
            if cheat.freecam_enabled and fc_pos then
                (function()
                    for _, plr in ipairs(Players:GetPlayers()) do
                        ...
                        if not char then
                            freecam_replication_cache[plr] = nil
                            continue
                        end
                        local cached = freecam_replication_cache[plr]
                        local hrp = cached and cached.character == char and cached.hrp or nil
                        if not hrp or not hrp.Parent then
                            hrp = char:FindFirstChild("HumanoidRootPart")
                            freecam_replication_cache[plr] = {character = char, hrp = hrp}
                        end
    ...
    local freecam_character, freecam_hrp
    cheat.utility.new_renderstepped((function(delta)
        local character = Players.LocalPlayer.Character
        if character ~= freecam_character then
            freecam_character = character
            freecam_hrp = character and _FindFirstChild(character, "HumanoidRootPart")
        end
        local hrp = freecam_hrp
...
do
    ...
    local move_character, move_humanoid, move_hrp
    cheat.utility.new_renderstepped((function()
        local c = Players.LocalPlayer.Character
        if c ~= move_character then
            move_character = c
            move_humanoid = c and c:FindFirstChildOfClass("Humanoid")
            move_hrp = c and c:FindFirstChild("HumanoidRootPart")
        end
        local hum = move_humanoid
        local hrp = move_hrp
        if hum then
...
local _cached_nocollision_refresh = 0
...
cheat.utility.new_heartbeat((function(delta)
    ...
    if tick() - _cached_nocollision_refresh > 1 then
        _cached_nocollision_refresh = tick()
        local current_nc = workspace:FindFirstChild("NoCollision")
        if current_nc ~= _cached_nocollision then
            _cached_nocollision = current_nc
        end
    end
