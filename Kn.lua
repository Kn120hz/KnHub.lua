...
local Window     = Library:Window({ Name = "tomboy.hook", Logo = logoAsset or "" })
local Watermark  = Window:Watermark("tomboy.hook")
    ...
    cheat.utility.new_renderstepped(LPH_NO_VIRTUALIZE(function()
        ...
        if plr then
            ...
            if tick() - target_info_last_scan >= 0.5 then
                ...
                -- Try the common report storage locations used by player data.
                local report_names = {
                    Reports=true, Report=true, ReportCount=true, ReportsCount=true,
                    TotalReports=true, ReceivedReports=true, ReceivedReportCount=true
                }
                local function find_report_count(root)
                    ...
                    for name in pairs(report_names) do
                        local attr = root:GetAttribute(name)
                        if type(attr) == "number" then
                            return math.floor(attr)
                        end
                    end
                ...
                target_info_reports = find_report_count(pdata)
            end
        ...
        TargetStats.Text = string.format(
            ...
            reports ~= nil and tostring(reports) or "--",
            mod_or_cheat
        )
    ...
    cheat.utility.new_renderstepped(LPH_NO_VIRTUALIZE(function()
        ...
        if silent_aim.target_line and silent_aim.target_part then
            ...
            if target_on_screen and target_screen.Z > 0 then
                ...
                local inside_fov = (not silent_aim.fov) or magnitude <= silent_aim.fov_size
                if inside_fov then
...
do
    ...
    local logotext   = cheat.utility.new_drawing("Text",{Visible=false,Font=Drawing.Fonts.Plex,Size=13,Color=Color3.fromRGB(138,128,255),ZIndex=3,Transparency=1,Text="tomboy.hook",Center=true,Outline=true})
...
cheat.utility.new_heartbeat(LPH_JIT_MAX(function(delta)
    ...
    -- Final consistency guard: when Use FOV is enabled, the selected part must
    -- remain inside the exact same center/radius used by the FOV drawing.
    if silent_aim.target_part and silent_aim.fov then
        local vp = Camera.ViewportSize
        ...
        local same_fov = onScreen and screen.Z > 0 and (screenPos - center).Magnitude <= silent_aim.fov_size
        ...
        for _, obj in ipairs(info.objectives) do
            ...
            if quest_esp_enabled and obj.position and not obj.done then
                ...
                if visible and screen.Z > 0 then
                    quest_lines[line_index].Text = tostring(obj.text) .. "  [" .. math.floor(screen.Z) .. "m]"
